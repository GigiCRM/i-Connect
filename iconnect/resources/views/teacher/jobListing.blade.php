...
<div class="row" id="row-adjust" style="float:right; display:block; padding-bottom:10px;" >
            <form action="{{ route('teacher.searchJobList') }}" method="post"  id="search">
                @csrf
		    ...
		    <table class="table table-hover table-striped">
                ...
                @foreach($job as $jobs)
		            <tr>
                        ...
                        <td>{{$jobs->status}}</td>
                        <td>
                        <a href="{{route('teacher.editJob', ['id' => $jobs->id])}}" class="btn btn-secondary">Edit</a>
                        <a href="{{route('teacher.deleteJob', ['id' => $jobs->id])}}" class="btn btn-danger" onclick="return confirm('Sure Want Delete?')">Delete</a>
